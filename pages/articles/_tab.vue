  ...
  <div ref="article" class="article-main">
    <ul class="articles-list">
      <li v-for="(item, idx) in list " :key="idx" class="item" @click="linkDetail(item.id)">
        ...
        <div v-if="item.top" class="label green">{{ tabs['top'] }}</div>
        <div v-else-if="item.good" class="label green">{{ tabs['good'] }}</div>
        <div v-else class="label gray">{{ tabs[item.tab] }}</div>
        <div class="text">
...
export default {
  ...
  async fetch({ store, params }) {
    ...
    this.page = 0;
    await store.dispatch(`articles/${types.CHANGE_LIST.toLowerCase()}`, {
      tab: params.tab,
      page: 1
    });
    if (loadingInstance) {
  ...
  mounted() {
    this.$refs.article.addEventListener(
      "scroll",
      _.throttle(this.computedScroll, 300)
    );
  },
  methods: {
    linkDetail(id) {
